...
class attendenceController extends Controller
{
    public function attendence_sheet()
    {
        ...
        $data['class'] = '';
        $data['date_month'] = '';
        return view('dashboard/attendence.attendence_sheet', $data);
    }
    ...
    {        $school_id = sitedetails()[0]->school_id;
        ...
        if ($data['data'] == 'Daily') {
            ...
            $data['rows'] = DB::table('attendances')->where($aWhere)->orderBy('date','ASC')->get();
            $sWhere = [
                'StudentClass' => $student_class,
                'Year' => date('Y'),
                'StudentStatus' => 'Active',
...
return view('dashboard/attendence.att_edit',$data);
